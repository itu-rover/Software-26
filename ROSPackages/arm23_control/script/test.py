...
class NaimDrive():
    ...
    async def flush(self):
        try:
            await asyncio.wait_for(self.fdcanusb.read(), 0.05)
        ...
        while not rospy.is_shutdown():
            try:
                if self.reset_timeout:
                    await asyncio.wait_for(self.c1.set_stop())
                    await asyncio.wait_for(self.c2.set_stop())
                    await asyncio.wait_for(self.c3.set_stop())
                    await asyncio.wait_for(self.c4.set_stop())
                    await asyncio.wait_for(self.c5.set_stop())
                    await asyncio.wait_for(self.c6.set_stop())
                    self.reset_timeout = False

                if self.VELOCITY_MODE:
                    rospy.loginfo("VELOCITY")
                    command1 = await asyncio.wait_for(self.c1.set_position(position=math.nan, velocity=self.velocity_command[0], accel_limit=self.acc_limit, maximum_torque = 4, query=True), 0.01)
                    command2 = await asyncio.wait_for(self.c2.set_position(position=math.nan, velocity=self.velocity_command[1], accel_limit=self.acc_limit, maximum_torque = 3, query=True), 0.01)
                    command3 = await asyncio.wait_for(self.c3.set_position(position=math.nan, velocity=self.velocity_command[2], accel_limit=self.acc_limit, maximum_torque = 3, query=True), 0.01)
                    command4 = await asyncio.wait_for(self.c4.set_position(position=math.nan, velocity=self.velocity_command[3], accel_limit=self.acc_limit, maximum_torque = 3, query=True), 0.01)
                    command5 = await asyncio.wait_for(self.c5.set_position(position=math.nan, velocity=self.velocity_command[4], accel_limit=self.acc_limit, maximum_torque = 3, query=True), 0.01)
                    command6 = await asyncio.wait_for(self.c6.set_position(position=math.nan, velocity=self.velocity_command[5], accel_limit=self.acc_limit, maximum_torque = 3, query=True), 0.01)

                elif not self.ALLOW_WRITE:
                    rospy.loginfo("FEEDBACK")
                    rospy.logwarn("Moteus 1")
                    command1 = await asyncio.wait_for(self.c1.set_position(position=math.nan,
                                                        query = True),0.01)
                    rospy.logwarn("Moteus 2")
                    command2 = await asyncio.wait_for(self.c2.set_position(position=math.nan,
                                                        query = True),0.01)
                    rospy.logwarn("Moteus 3")
                    command3 = await asyncio.wait_for(self.c3.set_position(position=math.nan,
                                                        query = True),0.01)
                    rospy.logwarn("Moteus 4")
                    command4 = await asyncio.wait_for(self.c4.set_position(position=math.nan,
                                                        query = True),0.01)
                    rospy.logwarn("Moteus 5")
                    command5 = await asyncio.wait_for(self.c5.set_position(position=math.nan,
                                                        query = True),0.01)
                    rospy.logwarn("Moteus 6")
                ...
                rospy.loginfo(self.moteus_modes)

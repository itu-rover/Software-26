...
class TeleopClass:
    def __init__(self):
        self.k_linear = 0.0005*2.7
        self.k_angular = 0.001*2
        ...
        self.gripper_pub = rospy.Publisher("gripper_commands", Int16, queue_size=10)
    ...
    def joint_func(self):
        ...
        message_to_send = "S"
        message_to_send += str(round(max(1, min(5 + (self.data.axes[4] * self.data.buttons[0]) * 5, 9))))
        ...
        message_to_send += str(round(max(1, min(5 + (self.data.axes[4] * self.data.buttons[3]) * 5, 9))))
        message_to_send += str(round(max(1, min(5 + (self.data.axes[4] * self.data.buttons[4]) * 5, 9))))
        message_to_send += str(round(max(1, min(5 + (self.data.axes[4] * self.data.buttons[5]) * 5, 9))))
        gripper_message = int(max(1, min(5 + (self.data.axes[4] * self.data.buttons[6]) * 5, 9)))
        message_to_send += "F"
    ...
    def cartesian_func(self):
        ...
        msg.twist.linear.x = deadband(self.data.axes[3], 0.3) * self.k_linear * (1-self.data.buttons[0]) * -speed_modifier

        msg.twist.linear.y = deadband(-self.data.axes[4], 0.3) * self.k_linear * (1-self.data.buttons[0]) * -speed_modifier

        msg.twist.linear.z = deadband(self.data.axes[1], 0.3) * self.k_linear * (1-self.data.buttons[0]) * speed_modifier

        msg.twist.angular.x = deadband(-self.data.axes[1], 0.2) * self.k_angular * self.data.buttons[0] * 5 * speed_modifier

        msg.twist.angular.y = deadband(-self.data.axes[4], 0.2) * self.k_angular * self.data.buttons[0] * 5 * speed_modifier

        msg.twist.angular.z = deadband(self.data.axes[0], 0.2) * self.k_angular * self.data.buttons[0] * 5 * speed_modifier

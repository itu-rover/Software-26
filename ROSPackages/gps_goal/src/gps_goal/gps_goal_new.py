...
class GpsGoal():
  def __init__(self):
    ...
    rospy.Subscriber('/clap/odom', Odometry, self.clap_cb)
  ...
  def gps_goal_pose_callback(self, data): 
    lat = data.pose.position.x #y
    long = data.pose.position.y #x
  ...
  def publish_goal(self, x=0, y=0, z=0, yaw=0, roll=0, pitch=0):
    # Create move_base goal
    goal = MoveBaseGoal()
    goal.target_pose.header.frame_id = 'odom' #rospy.get_param('~frame_id','odom')
    goal.target_pose.pose.position.x = x + self.odom.pose.pose.position.x #y
    goal.target_pose.pose.position.y = y + self.odom.pose.pose.position.y #x
    ...
    # goal.pose.position.x = x + self.odom.pose.pose.position.x
    # goal.pose.position.y = y + self.odom.pose.pose.position.y
...
#gps_goal.py --lat 29.017774 --long 41.10283